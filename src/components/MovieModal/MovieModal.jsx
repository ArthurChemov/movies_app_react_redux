  ...
  React.useEffect(() => {
    API.get(`/movie/${movieId}`)
      .then((response) => {
        ...
        setGenres(response.data.genres.map((genre, i) => `${genre.name}${response.data.genres.length > ++i ? ', ' : ''}`));
    ...
    API.get(`/movie/${movieId}/credits`)
      .then((response) => {
        setCast(response.data.cast.map((person, i) => <CastTile key={i} person={person}/>));
      });
